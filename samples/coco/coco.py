...
DEFAULT_LOGS_DIR = os.path.join('trained_model/coco/', "logs")
...
class CocoDataset(utils.Dataset):
    ...
    def load_structure_information_from_boxes(self, boxes):
        ...
        return np.asarray(start_rows), np.asarray(start_cols), np.asarray(end_rows), np.asarray(end_cols)
...
def evaluate_coco(model, dataset, coco, eval_type="bbox", limit=0, image_ids=None):
    ...
    for i, image_id in enumerate(image_ids):
        ...
        image_name = "AR_"+str(coco_image_ids[i])+".jpg"
        result_path = os.path.join('trained_model/coco/result_jpg/', image_name)
        result_col_path = os.path.join('trained_model/coco/result_col_pkl/', "AR_"+str(coco_image_ids[i])+"_col.pkl")
        result_row_path = os.path.join('trained_model/coco/result_row_pkl/', "AR_"+str(coco_image_ids[i])+"_row.pkl")
        ...
        f.close()
        output=visualize.display_instances(image_name, image, r['rois'], r['masks'], r['class_ids'],dataset.class_names, r['scores'])
