...
def refine_structure_detections_graph(image_features,
    ...
    roi_iou_max = tf.reduce_max(overlaps, axis=1)
    # 1. Positive ROIs are those with >= 0.5 IoU with a GT box
    positive_roi_bool = (roi_iou_max >= 0.25)
    positive_indices = tf.where(positive_roi_bool)[:, 0]
    # 2. Negative ROIs are those with < 0.5 with every GT box. Skip crowds.
    positive_overlaps = tf.gather(overlaps, positive_indices)
    roi_gt_box_assignment = tf.cond(
...
def edge_features(vertices_in, num_neighbors=10):
    ...
    def indexing_tensor(spatial_features, k=10, n_batch=-1):
        ...
        batch_range = tf.expand_dims(tf.expand_dims(
            tf.expand_dims(tf.range(0, n_batch), axis=1), axis=1), axis=1)
        batch_range = tf.tile(batch_range, [1, n_max_entries, k, 1])
        expanded_neighbor_matrix = tf.expand_dims(neighbor_matrix, axis=3)

        indexing_tensor = tf.concat(
            [batch_range, expanded_neighbor_matrix], axis=3)
    ...
    expanded_trans_space = tf.tile(
        expanded_trans_space, [1, 1, num_neighbors, 1])
    diff = expanded_trans_space - neighbour_space
    edge = tf.concat([expanded_trans_space, diff], axis=-1)
    return edge


def edge_conv_layer_row(vertices_in, num_neighbors=30, mpl_layers=[128, 128, 128], aggregation_function=tf.reduce_max, edge_activation=None, layer_index=0):
    edge = KL.Lambda(lambda x: edge_features(
        x, num_neighbors), name='graph_edge_conv_layer_'+str(layer_index+1)+'_row')(vertices_in)
    for i in range(len(mpl_layers)):
        f = mpl_layers[i]
        edge = KL.Dense(f, activation='relu',
                        name='graph_edge_conv_layer_'+str(layer_index+1)+'_row_dense_' + str(i+1))(edge)
    if edge_activation is not None:
        edge = KL.Lambda(lambda x: edge_activation(
            x), name='graph_edge_conv_layer_'+str(layer_index+1)+'_row_edge_activation')(edge)
    vertex_out = KL.Lambda(lambda x: aggregation_function(
        x, axis=2), name='graph_edge_conv_layer_'+str(layer_index+1)+'_row_edge_aggregation')(edge)
    return vertex_out


def edge_conv_layer_col(vertices_in, num_neighbors=30, mpl_layers=[128, 128, 128], aggregation_function=tf.reduce_max, edge_activation=None, layer_index=0):
    edge = KL.Lambda(lambda x: edge_features(
        x, num_neighbors), name='graph_edge_conv_layer_'+str(layer_index+1)+'_col')(vertices_in)
    for i in range(len(mpl_layers)):
        f = mpl_layers[i]
        edge = KL.Dense(f, activation='relu',
                        name='graph_edge_conv_layer_'+str(layer_index+1)+'_col_dense_' + str(i+1))(edge)
    if edge_activation is not None:
        edge = KL.Lambda(lambda x: edge_activation(
            x), name='graph_edge_conv_layer_'+str(layer_index+1)+'_col_edge_activation')(edge)
    vertex_out = KL.Lambda(lambda x: aggregation_function(
        x, axis=2), name='graph_edge_conv_layer_'+str(layer_index+1)+'_col_edge_aggregation')(edge)
    return vertex_out


def high_dim_dense_row(inputs, nodes, layer_index=0):
    if len(inputs.shape) == 3:
        return KL.Conv1D(nodes, kernel_size=(1), strides=(1), padding='valid', name="graph_high_dim_layer_conv1d_row_"+str(layer_index+1))(inputs)
    if len(inputs.shape) == 4:
        return KL.Conv2D(nodes, kernel_size=(1, 1), strides=(1, 1), padding='valid', name="graph_high_dim_layer_conv2d_row_"+str(layer_index+1))(inputs)
    if len(inputs.shape) == 5:
        return KL.Conv3D(nodes, kernel_size=(1, 1, 1), strides=(1, 1, 1), padding='valid', name="graph_high_dim_layer_conv3d_row_"+str(layer_index+1))(inputs)


def high_dim_dense_col(inputs, nodes, layer_index=0):
    if len(inputs.shape) == 3:
        return KL.Conv1D(nodes, kernel_size=(1), strides=(1), padding='valid', name="graph_high_dim_layer_conv1d_col_"+str(layer_index+1))(inputs)
    if len(inputs.shape) == 4:
        return KL.Conv2D(nodes, kernel_size=(1, 1), strides=(1, 1), padding='valid', name="graph_high_dim_layer_conv2d_col_"+str(layer_index+1))(inputs)
    if len(inputs.shape) == 5:
        return KL.Conv3D(nodes, kernel_size=(1, 1, 1), strides=(1, 1, 1), padding='valid', name="graph_high_dim_layer_conv3d_col_"+str(layer_index+1))(inputs)


def prepare_graph_features_row(vertex_features, train_flag, num_neighbors=10):
    feat = KL.BatchNormalization(momentum=0.8,
                                 trainable=train_flag,
                                 name='graph_feat_row_batch_norm')(vertex_features)
    feat = high_dim_dense_row(feat, 128, 0)
    feat = edge_conv_layer_row(feat, num_neighbors, [
                               128, 128, 128], layer_index=0)
    ...
    feat1 = edge_conv_layer_row(
        feat, num_neighbors, [128, 128, 128], layer_index=1)
    ...
    feat2 = edge_conv_layer_row(feat1, num_neighbors, [
                                128, 128, 128], layer_index=2)
    ...
    feat3 = edge_conv_layer_row(feat2, num_neighbors, [
                                128, 128, 128], layer_index=3)
    feat = KL.Concatenate()(
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            aggregate_vertex_features_row = prepare_graph_features_row(
                vertex_features_row, True, num_neighbors=config.GRAPH_NEIGHBORS)
            # aggregate_vertex_features_row = prepare_graph_features_row(
            #     vertex_features_row,
            ...
            aggregate_vertex_features_col = prepare_graph_features_col(
                vertex_features_col, True, num_neighbors=config.GRAPH_NEIGHBORS)
            # aggregate_vertex_features_col = prepare_graph_features_col(
            #     vertex_features_col,
            #     n_neighbours=config.GRAPH_NEIGHBORS,
            #     n_dimensions=8,
            ...
            aggregate_vertex_features_row = prepare_graph_features_row(
                vertex_features_row, False, num_neighbors=config.GRAPH_NEIGHBORS)
            # aggregate_vertex_features_row = prepare_graph_features_row(
            #     vertex_features_row,
            #     n_neighbours=config.GRAPH_NEIGHBORS,
            #     n_dimensions=8,
            ...
            #     n_propagate=32)
            aggregate_vertex_features_col = prepare_graph_features_col(
                vertex_features_col, False, num_neighbors=config.GRAPH_NEIGHBORS)
            # aggregate_vertex_features_col = prepare_graph_features_col(
            #     vertex_features_col,
            #     n_neighbours=config.GRAPH_NEIGHBORS,
            #     n_dimensions=8,
